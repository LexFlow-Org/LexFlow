...
pub const VAULT_V4_MAGIC: &[u8] = b"LEXFLOW_V4";
const AES_KEY_LEN: usize = 32;
const NONCE_LEN: usize = 12;
pub const MAX_RECORD_VERSIONS: usize = 5;
...
pub fn wrap_dek(kek: &[u8], dek: &[u8]) -> Result<(String, String), String> {
...
pub fn compute_header_mac(kek: &[u8], vault: &VaultV4) -> String {
...
pub fn encrypt_record(dek: &[u8], plaintext: &[u8]) -> Result<EncryptedBlock, String> {
...
pub fn decrypt_record(dek: &[u8], block: &EncryptedBlock) -> Result<Vec<u8>, String> {
...
pub fn encrypt_index(dek: &[u8], entries: &[IndexEntry]) -> Result<EncryptedBlock, String> {
    let plaintext = serde_json::to_vec(entries).map_err(|e| format!("Index serialize: {}", e))?;
    encrypt_record(dek, &plaintext)
}

/// Decrypt the index.
pub fn decrypt_index(dek: &[u8], block: &EncryptedBlock) -> Result<Vec<IndexEntry>, String> {
...
pub fn read_current_version(entry: &RecordEntry, dek: &[u8]) -> Result<Vec<u8>, String> {
    let version = entry
        .versions
        .iter()
...
pub fn needs_rotation(rotation: &RotationMeta) -> bool {
...
pub fn rotate_dek(vault: &mut VaultV4, kek: &[u8]) -> Result<Zeroizing<Vec<u8>>, String> {
    // Unwrap old DEK
...
pub fn detect_vault_version(data: &[u8]) -> u32 {
    ...
    if data.starts_with(b"LEXFLOW_V2_SECURE") {
        return 2;
    }
    0 // unknown
}
...
pub fn migrate_v2_to_v4(
    ...
    let v2_key = derive_secure_key_legacy(password, v2_salt)?;
    let decrypted = {
        use aes_gcm::{aead::{Aead as _, KeyInit as _, Payload as GcmPayload}, Aes256Gcm, Key as GcmKey, Nonce as GcmNonce};
        let magic = b"LEXFLOW_V2_SECURE";
        if v2_data.len() < magic.len() + NONCE_LEN + 16 { return Err("Corrupted v2".into()); }
        if !v2_data.starts_with(magic) { return Err("Not a v2 vault".into()); }
        let nonce = GcmNonce::from_slice(&v2_data[magic.len()..magic.len() + NONCE_LEN]);
        let ct = &v2_data[magic.len() + NONCE_LEN..];
        let cipher = Aes256Gcm::new(GcmKey::<Aes256Gcm>::from_slice(&v2_key));
        let payload = GcmPayload { msg: ct, aad: magic };
        cipher.decrypt(nonce, payload).map_err(|_| "V2 decrypt failed".to_string())?
    };
...
pub fn extract_record_title_pub(item: &serde_json::Value, field: &str) -> String {
    extract_record_title(item, field)
}

/// Extract tags from a record (pub for vault.rs).
pub fn extract_record_tags_pub(item: &serde_json::Value, field: &str) -> Vec<String> {
...
pub fn generate_recovery_key(vault: &mut VaultV4, dek: &[u8]) -> Result<String, String> {
...
// ─── Legacy v2 helpers (for migration tests) ────────────────

pub fn derive_secure_key_legacy(password: &str, salt: &[u8]) -> Result<Zeroizing<Vec<u8>>, String> {
    let mut key = Zeroizing::new(vec![0u8; AES_KEY_LEN]);
    let params = Params::new(16384, 3, 1, Some(AES_KEY_LEN)).map_err(|e| e.to_string())?;
    let argon2 = Argon2::new(Algorithm::Argon2id, Version::V0x13, params);
    let pwd_bytes = Zeroizing::new(password.as_bytes().to_vec());
...
pub fn encrypt_data_legacy(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, String> {
    use aes_gcm::{aead::{Aead as _, KeyInit as _, Payload as GcmPayload}, Aes256Gcm, Key as GcmKey, Nonce as GcmNonce};
    let magic = b"LEXFLOW_V2_SECURE";
    let cipher = Aes256Gcm::new(GcmKey::<Aes256Gcm>::from_slice(key));
    let mut nonce_bytes = [0u8; NONCE_LEN];
    rand::RngCore::fill_bytes(&mut rand::rngs::OsRng, &mut nonce_bytes);
    let payload = GcmPayload { msg: plaintext, aad: magic };
    let ciphertext = cipher.encrypt(GcmNonce::from_slice(&nonce_bytes), payload).map_err(|_| "Encryption error")?;
