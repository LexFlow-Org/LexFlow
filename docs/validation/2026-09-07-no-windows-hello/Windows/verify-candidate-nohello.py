...
import hashlib,json,struct,subprocess,shutil,re,datetime,sys
...
output=root/'candidate-nohello';output.mkdir(exist_ok=True)
extracted=root/'extracted-nohello'
...
(root/'installer-7z-test-nohello.log').write_text(check.stdout+check.stderr)
assert check.returncode==0,check.stdout+check.stderr
extract=subprocess.run(['7z','x',str(installer),'-o'+str(extracted),'-y'],capture_output=True,text=True)
(root/'installer-extract-nohello.log').write_text(extract.stdout+extract.stderr)
...
binary=(extracted/'lexflow.exe').read_bytes()
removed_tokens=['UserConsentVerifier','RequestVerificationAsync','Windows.Security.Credentials.UI','WindowsRuntimeSystemExtensions']
removed_token_results={token:all(token.encode(encoding) not in binary for encoding in ['utf-8','utf-16le']) for token in removed_tokens}
assert all(removed_token_results.values()),removed_token_results
bio_source=(project/'src-tauri/src/bio.rs').read_text()
assert 'UserConsentVerifier' not in bio_source and 'RequestVerificationAsync' not in bio_source
assert 'pub(crate) fn remove_legacy_windows_bio' in bio_source
assert 'windows_hello_removed' in bio_source
...
original_source=Path(sys.argv[1]).resolve()
for name,digest in source.items():assert sha(original_source/name)==digest,('original source changed',name)
frontend_source=json.loads((root/'frontend-source-sha256.json').read_text())
for name,digest in frontend_source.items():assert sha(original_source/name)==digest,('frontend source changed',name)
frontend_dist=[p for p in (project/'client/dist').rglob('*') if p.is_file()]
assert len(frontend_dist)==27
assert all(' 2.' not in p.name for p in frontend_dist)
revoked_public_key=bytes.fromhex('5a0721069b92eee3db40d1b21545b15ab57fe7e99001365b5e71bcf4a8221f0e')
assert revoked_public_key not in binary
nsis_files=list((release/'nsis').glob('*.nsi'))+list(release.glob('**/installer.nsi'))
receipt={
 'date':datetime.datetime.now().astimezone().date().isoformat(),'built_at':datetime.datetime.now().astimezone().isoformat(),'product':'LexFlow','version':'1.0.1','target':'x86_64-pc-windows-msvc',
 ...
 'verification':{'installer_7z_test':'passed','installer_extraction':'passed','extracted_files':sum(p.is_file() for p in extracted.rglob('*')),'application_resource_files_compared_sha256':len(expected),'webview2_fixed_runtime_files_compared_sha256':len(runtime_files),'vendor_qpdf_dlls':9,'pe':pe,'executable_sha256':expected['lexflow.exe'],'public_key_sha256':hashlib.sha256(public_key).hexdigest(),'public_key_bytes_present':True,'revoked_public_key_present':False,'current_project_matches_frozen_snapshot':True,'source_files_compared':len(source),'frontend_source_files_compared':len(frontend_source),'frontend_dist_files':len(frontend_dist),'frontend_duplicate_suffix_files':[],'removed_windows_hello_api_tokens_absent_utf8_utf16':removed_token_results,'legacy_credential_cleanup_function_present_in_source':True,'native_windows_execution':False,'windows_network_capture':False,'webview2_appcontainer_acl_hook':{'source':'src-tauri/windows/installer-hooks.nsh','sha256':sha(backend/'windows/installer-hooks.nsh'),'scope':'$INSTDIR\\binaries\\windows-webview2','grants':['S-1-15-2-2:(OI)(CI)(RX)','S-1-15-2-1:(OI)(CI)(RX)'],'failure_handling':'MessageBox + SetErrorLevel1 + Abort on command/exit failure','validation':'NSIS installer compiled; icacls and Windows app were not executed on macOS'}},
 'source_sha256_at_build':source,
 'frontend_source_sha256_at_build':json.loads((root/'frontend-source-sha256.json').read_text()),
 'change_scope':'Windows Hello removed: password-only Windows unlock, no enrollment/read/consent subprocess. One-time legacy Credential Manager entry deletion and marker cleanup. Windows version unchanged 1.0.1.',
