...
manifest={}
for prefix in ['src-tauri/src','src-tauri/capabilities','src-tauri/windows','client/dist']:
...
frontend={}
for file in sorted((source/'client/src').rglob('*')):
    if file.is_file():frontend[str(file.relative_to(source))]=hashlib.sha256(file.read_bytes()).hexdigest()
(root/'frontend-source-sha256.json').write_text(json.dumps(frontend,indent=2)+'\n')
