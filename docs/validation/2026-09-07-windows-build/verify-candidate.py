...
output=root/'candidate';output.mkdir(exist_ok=True)
...
(root/'installer-7z-test.log').write_text(check.stdout+check.stderr)
assert check.returncode==0,check.stdout+check.stderr
extract=subprocess.run(['7z','x',str(installer),'-o'+str(extracted),'-y'],capture_output=True,text=True)
(root/'installer-extract.log').write_text(extract.stdout+extract.stderr)
