...
L'APK mantiene il certificato Android della versione 1.0.0 e della precedente 1.0.1 e aumenta il codice versione a 253. Su un dispositivo dove era installato quell'APK, installa il nuovo pacchetto come aggiornamento senza disinstallare prima l'applicazione. La build253 aggiunge la correzione degli interruttori nelle impostazioni e una diagnostica più precisa della disponibilità biometrica. Include inoltre le correzioni precedenti alla visibilità del modulo password sui display stretti e agli stili bloccati dalla CSP; disabilita inoltre esplicitamente la diagnostica WebView nelle build release. Il passaggio da una versione all'altra va verificato con dati fittizi prima dell'uso professionale.
...
`SHA256SUMS.txt` contiene l'impronta del pacchetto. `CERTIFICATO-FIRMA.txt` identifica il certificato pubblico usato per questa release. La chiave privata di firma è conservata separatamente dal proprietario del progetto e non deve essere inviata insieme all'applicazione. Gli aggiornamenti Android devono mantenere lo stesso certificato.

Il precedente APK252 è stato installato come aggiornamento nell'emulatore AOSP API35: quattro riaperture offline sono riuscite e il modulo password è visibile. Con il renderer software SwiftShader, durante un avvio in cui anche System UI si è bloccata, si è riprodotto un arresto del runtime Wry per Timeout. La causa non è stata corretta. Restano inoltre bande grafiche nel renderer automatico e il collaudo completo di archivio e backup Android da eseguire. Questa build non viene presentata come validata nell'uso professionale su Android.

Il pacchetto253 è stato ricompilato e verificato per firma, manifest, risorse e allineamento. Non è stato eseguito nuovamente su emulatore o telefono: le prove del pacchetto252 non costituiscono collaudo runtime di questa nuova build. I limiti precedentemente osservati restano aperti.
