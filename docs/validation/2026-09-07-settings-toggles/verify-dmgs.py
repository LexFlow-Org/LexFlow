...
for label,arch in [('Apple-Silicon','arm64'),('Intel','x86_64')]:
    dmg=root/'candidate-toggle'/f'LexFlow-1.0.1-macOS-{label}.dmg'
...
(root/'candidate-toggle'/'verifica-pacchetto.json').write_text(json.dumps({'date':'2026-09-07','artifacts':artifacts,'source_freeze_sha256':hashlib.sha256((Path('/private/tmp/lexflow-ui-update-2026-09-07/source-freeze.json')).read_bytes()).hexdigest(),'rust_tests_passed':428,'native_gui_intermediate_arm_tested':True,'intel_native_hardware_tested':False},indent=2)+'\n')
